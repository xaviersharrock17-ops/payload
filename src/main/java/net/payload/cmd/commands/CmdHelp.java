...
public class CmdHelp extends Command {
    ...
    public void runCommand(String[] parameters) {
        if (parameters.length <= 0) {
            ...
            Module module = Payload.getInstance().moduleManager.getModuleByName(parameters[0]);
            if (module == null) {
    ...
    private void ShowCommands(int page) {
        ...
        CommandManager.sendChatMessage(title);
        CommandManager.sendChatMessage("Use " + Formatting.LIGHT_PURPLE + ".payload help [n]" + Formatting.RESET + " to get page n of help.");

        // Fetch the commands and dislays their syntax on the screen.
        Map<String, Command> commands = Payload.getInstance().commandManager.getCommands();
        ...
        for (int i = (page - 1) * indexesPerPage; i <= (page * indexesPerPage); i++) {
            if (i >= 0 && i < Payload.getInstance().commandManager.getNumOfCommands()) {
                CommandManager.sendChatMessage(" .payload " + listOfCommands.get(i));
            }
        }
    ...
    public String[] getAutocorrect(String previousParameter) {
        CommandManager cm = Payload.getInstance().commandManager;
        int numCmds = cm.getNumOfCommands();
        String[] commands = new String[numCmds];

        Set<String> cmds = Payload.getInstance().commandManager.getCommands().keySet();
        int i = 0;
        for (String x : cmds)
            commands[i++] = x;
