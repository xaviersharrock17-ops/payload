...
public class EnumComponent<T extends Enum<T>> extends Component {
	...
	public void draw(DrawContext drawContext, float partialTicks) {
		...
		switch (GuiManager.modulesettingsstyle.get()) {
			case Collapsed:
				String displayText = hovered ? enumValue: enumSetting.displayName;
				float displayTextWidth = Render2D.getStringWidth(displayText);

				Render2D.drawString(drawContext, displayText,
						actualX + (actualWidth / 2.0f) - displayTextWidth,
			...
			case Popout:
				...
				Render2D.drawString(drawContext, enumValue,
						actualX + (actualWidth / 2.0f) - stringWidth + 61,
						Math.round(actualY + 13),  // Rounded Y position
						0x9B9B9B);
				break;
		}
	}
