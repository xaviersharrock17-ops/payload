...
public class ExplosionUtil {
    public static float anchorDamage(BlockPos pos, PlayerEntity target, PlayerEntity predict){
        if (OtherBlockUtils.getBlock(pos) == Blocks.RESPAWN_ANCHOR) {
    ...
    public static float getBlastReduction(LivingEntity entity, float damageI) {
        ...
        if (entity instanceof PlayerEntity player) {
            PlayerManager.EntityAttribute a = Payload.getInstance().playerManager.map.get(player);
            if (a == null) return 0;
...
/*            if (player.hasStatusEffect(StatusEffects.RESISTANCE)) {
    ...
    public static int getProtectionAmount(Iterable<ItemStack> armorItems) {
        ...
        for (ItemStack itemStack : new ArrayList<>((DefaultedList<ItemStack>) armorItems)) {
            ...
            int level = EnchantmentHelper.getLevel(MC.world.getRegistryManager().getOrThrow(Enchantments.PROTECTION.getRegistryRef()).getEntry(Enchantments.PROTECTION.getValue()).orElse(null), itemStack);
            if (level == 0) {
                value += EnchantmentHelper.getLevel(MC.world.getRegistryManager().getOrThrow(Enchantments.BLAST_PROTECTION.getRegistryRef()).getEntry(Enchantments.BLAST_PROTECTION.getValue()).orElse(null), itemStack) * 2;
            } else {
                value += level;
            }
        }
