package net.payload.utils.bowaimbot;
...
public class BowAimbotTargets {
    protected static final PayloadClient PAYLOAD_CLIENT = Payload.getInstance();
    ...
    public static final Predicate<Entity> IS_ATTACKABLE = e -> e != null
            ...
            && !PAYLOAD_CLIENT.friendsList.contains(e.getUuid());
    ...
    public static final Predicate<PlayerEntity> IS_PLAYER =
            p -> p != null && !p.isRemoved() && p.getHealth() > 0 && !PAYLOAD_CLIENT.friendsList.contains(p.getUuid()) && p != MC.player && !(p instanceof FakePlayerEntity);
