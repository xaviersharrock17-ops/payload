...
import net.payload.event.events.SendMovementPacketEvent.Post;
import net.payload.event.events.SendMovementPacketEvent.Pre;
...
public class Fly extends Module implements TickListener, SendMovementPacketListener {
	...
	private FloatSetting sprintSpeedMultiplier = FloatSetting.builder().id("fly_sprint_speed_multiplier")
			.displayName("Sprint Multiplier").description("Speed multiplier when sprinting.").defaultValue(1.5f)
			.minValue(1.0f).maxValue(3.0f).step(0.1f).build();

	private FloatSetting jumpMotionY = FloatSetting.builder().id("fly_jump_motion_y").displayName("Up Speed")
			.description("Upward motion when jump key is pressed.").defaultValue(0.5f).minValue(0.1f).maxValue(2.0f)
			.step(0.1f).build();

	private FloatSetting sneakMotionY = FloatSetting.builder().id("fly_sneak_motion_y").displayName("Down Speed")
			.description("Downward motion when sneak key is pressed.").defaultValue(-0.5f).minValue(-2.0f).maxValue(0f)
			.step(0.1f).build();
	...
	public Fly() {
		...
		this.setDescription("Vanilla flight like creative mode");
